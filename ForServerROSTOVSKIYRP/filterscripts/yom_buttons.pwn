...
public FS_CreateButton(Float:X, Float:Y, Float:Z, Float:Angle)
{
	...
	CreateDynamic3DTextLabel("F", 0xFFFF00EE, X, Y, Z + 0.15, 3.5);
...
public FS_DestroyButton(buttonid)
{
	if (FS_IsValidButton(buttonid))
	{
		...
		DestroyObject(ButtonInfo[buttonid][ObjectID]);
	}
}
/*----------------------------------------------------------------------------*/
...
forward FS_PlayerUseButton(playerid);
public FS_PlayerUseButton(playerid)
{
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	if (GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
	{
		...
		else if (newkeys & KEY_SPRINT)
		{
			if (PlayerInfo[playerid][SelectedButton] != INVALID_BUTTON_ID)
		    {
				if(PlayerInfo[playerid][MType])
				{
					PlayerInfo[playerid][MType] = 0;
				}
				else PlayerInfo[playerid][MType] = 1;
			}
		}
	    ...
	    if (PlayerKeys[0] + PlayerKeys[1] + PlayerKeys[2] != 0 && PlayerKeys[0] != KEY_HANDBRAKE)
	    {
			...
			else
			{
	    		if (PlayerKeys[1] == KEY_UP)
	    		{
					if(PlayerInfo[playerid][MType])
					{
						ButtonInfo[buttonid][Pos][2] += Move_Sin;
					}
					else
					{
				...
				(
					String,
					sizeof(String),
					"~n~~n~~n~~n~~n~~n~~n~~n~~w~Moving buttonid ~b~~h~%d ~n~~g~%s ~n~~w~X=~y~%.2f ~w~Y=~y~%.2f ~w~Z=~y~%.2f",
					buttonid,
					PlayerInfo[playerid][MType] ? ("vertical") : ("horizontal"),
					ButtonInfo[buttonid][Pos][0],
					ButtonInfo[buttonid][Pos][1],
					ButtonInfo[buttonid][Pos][2]
				);
