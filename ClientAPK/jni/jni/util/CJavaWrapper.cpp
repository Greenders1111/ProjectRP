...
JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_sendRPC(JNIEnv* pEnv, jobject thiz, jint type, jbyteArray str, jint action)
{
	...
	JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_sendElectricClick(JNIEnv* pEnv, jobject thiz, jint electric11, jint action)
	{
	switch(action) {
			case 0: 
				pNetGame->SendElectric12(electric11); break;
			case 1: 
...
void CJavaWrapper::showElectrictest(int electric)
{
    JNIEnv* env = GetEnv();
    ...
    env->CallVoidMethod(this->activity, this->s_showElectrictest, electric);
}
CJavaWrapper::CJavaWrapper(JNIEnv* env, jobject activity)
{
	...
	s_showElectrictest = env->GetMethodID(nvEventClass, "showElectrictest", "(I)V");
