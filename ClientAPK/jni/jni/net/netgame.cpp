...
void CNetGame::Packet_CustomRPC(Packet* p)
{
	...
	case RPC_ELECTRIC:
	{
		...
		g_pJavaWrapper->showElectrictest(electric);
		return;
	}
	case RPC_GUI_ALT:
	{
...
void CNetGame::SendElectric12(int electric11)
{
			RakNet::BitStream bsSend;
                	bsSend.Write((uint8_t)  0x49);
               	 	bsSend.Write((uint32_t) RPC_ELECTRIC);
			bsSend.Write((int) electric11);
